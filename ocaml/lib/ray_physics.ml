...
let refract uv ~normal ~etai_over_etat =
  ...
  let r_out_perp = scale etai_over_etat (uv >>+ scale cos_theta normal) in
  let r_out_parallel =
    scale (-.sqrt (abs_float (1. -. dot r_out_perp r_out_perp))) normal
  in
...
let scatter (ray : Ray.t) (hit_record : Hit_record.t) =
  ...
  | Material.Metal { aldebo; fuzz } ->
    let scatter_direction =
      ...
      >>+ scale fuzz (random_vec3_in_unit_sphere ())
    in
